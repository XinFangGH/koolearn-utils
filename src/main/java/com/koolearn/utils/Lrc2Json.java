 ...
 */
public class Lrc2Json {


    public static void lrc2JsonStart() {
    ...
    private static void writeJson(File file, String encoded) throws Exception {
        ...
        while ((tempStr = reader.readLine()) != null) {
            ...
            if (tempStr.startsWith("[") && tempStr.toCharArray().length > 10) {
                ...
                if (parse != null) {
                    ...
                    //时间
                    int time = parse.get(ChronoField.SECOND_OF_DAY);
                    srtJsonDomain.setStart(time);
                    srtJsonDomain.setSortNum(sortNum);
                    //计算end,end取下一个的start
                    if (sortNum > 1) {
                        SrtJsonDomain domain = list.get(sortNum - 2);
                        domain.setEnd(time);
                        list.remove(sortNum - 2);
                        list.add(domain);
                    }
                    sortNum++;
        ...
        SrtJsonDomain domain = list.get(list.size() - 1);
        domain.setEnd(domain.getStart() + 5);
